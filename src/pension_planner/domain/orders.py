...
class OrderBase(ABC):
    ...
    events: list[events.Event] = field(default_factory=list, init=False, compare=False, repr=False)
...
class SingleOrder(OrderBase):
    ...
    def get_timeseries(self) -> dict[datetime, float]:
        return {ts: i * self.amount
                for i, ts in enumerate(rrule(MONTHLY, dtstart=self.date, until=self.date), start=1)
                }
